/*
 * adapted from: https://github.com/glample/fastBPE
 * modified: Yunqi Li
 * contact: liyunqixa@gmail.com
 */
#ifndef _BPE_TOKENIZER_H_
#define _BPE_TOKENIZER_H_
...
/*
 * The BPE tokenizer only supports wstring.
 */
class BPETokenizer
{
private:
	static std::vector<wchar_t> puncsChinese;
	// static std::vector<char> puncsEnglish;
...
private:
	static std::wstring replacePuncs(const std::wstring &ws);
	static void getWordCount(const ChineseTable &chineseTable, std::unordered_map<std::wstring, uint32_t> &wordCount);
