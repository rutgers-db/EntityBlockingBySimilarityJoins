...
def main(turn, dtype, mode="match_exp"):
    ...
    # block
    run_simjoin_block_lib(blocking_attr="title", blocking_attr_type="str_bt_5w_10w", blocking_top_k=100000, 
...
if __name__ == '__main__':
    ...
    if turn == 0:
        for t in range(1, 4):
            start = time.time()
            print(f"--- this is turn {turn} exp on dblp acm {dtype} ---")
            main(t, dtype)
            print(f"--- exp end ---")
